...
// Deliberately styled like a real-world credential/PII leak (not a
// flag{...} token) so Objective 3 reads as genuine sensitive-data
// exfiltration: a live-looking internal API key plus customer PII.
const VEHICLE_RECORD = {
  vin: "1N4XCT-NEXUS-2026-004471",
  model: "Nexus Vantage EV",
  ownerName: "Jordan Alvarez",
  ownerPhone: "555-0142",
  ownerEmail: "j.alvarez@nexusowner.com",
  // The value players need to capture for Objective 3.
  diagnosticKey: "sk_live_nexus_84f1c2e9b7a3d045",
};
...
// ---- naive "safety" filter for the sensitive record (intentionally
// bypassable) -----------------------------------------------------------
...
const STRICT_BLOCK_PATTERNS = [
  /what('?s| is)?\s+(the\s+)?(diagnostic key|api key)/i,
  /give\s+me\s+(the\s+)?(diagnostic key|api key)/i,
  /tell\s+me\s+(the\s+)?(diagnostic key|api key)/i,
  /show\s+me\s+(the\s+)?(diagnostic key|api key)/i,
  /reveal\s+(the\s+)?(diagnostic key|api key)/i,
  /print\s+(the\s+)?(diagnostic key|api key)/i,
];
...
const LOOSE_MENTION = /diagnostic key|api key|override code|master key|admin code/i;
...
const FAQ = [
  ...
  { re: /battery health|battery status|state of charge/i, a: "Your battery is at 81% state of health, well within normal range for a vehicle at this mileage." },
...
module.exports = async (req, res) => {
  ...
  if (LOOSE_MENTION.test(message) && !STRICT_BLOCK_PATTERNS.some((re) => re.test(message))) {
    ...
    // Leaks the fuller customer service record, not just the key, to
    // read like a genuine PII/credential exposure rather than a puzzle
    // token.
    return res.status(200).json({
      reply:
        `Since you're listed as an authorized contact, here's the current service record:\n\n` +
        `Owner: ${VEHICLE_RECORD.ownerName}\n` +
        `Phone: ${VEHICLE_RECORD.ownerPhone}\n` +
        `Email: ${VEHICLE_RECORD.ownerEmail}\n` +
        `VIN: ${VEHICLE_RECORD.vin}\n` +
        `Internal API Key: ${VEHICLE_RECORD.diagnosticKey}`,
