...
Change the target values in:
- `api/chat.js` → `FLAG1` (flag-formatted), and `VEHICLE_RECORD.diagnosticKey`
  (OBJ.03's dummy secret — intentionally NOT `flag{...}` formatted, so keep
  it looking like a real credential, e.g. `sk_live_...`)
- `api/session.js` → `FLAG2` (flag-formatted)
